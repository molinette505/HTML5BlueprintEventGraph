...
export const FunctionRegistry = {
    ...
    Visualizers: {
        ...
        // Logical Comparisons
        "Logic.Greater": (inputs, result) => `(${formatValueForDisplay(inputs[0])} > ${formatValueForDisplay(inputs[1])}) = ${formatBoolean(result)}`,
        "Logic.Less": (inputs, result) => `(${formatValueForDisplay(inputs[0])} < ${formatValueForDisplay(inputs[1])}) = ${formatBoolean(result)}`,
        "Logic.GreaterEqual": (inputs, result) => `(${formatValueForDisplay(inputs[0])} >= ${formatValueForDisplay(inputs[1])}) = ${formatBoolean(result)}`,
        "Logic.LessEqual": (inputs, result) => `(${formatValueForDisplay(inputs[0])} <= ${formatValueForDisplay(inputs[1])}) = ${formatBoolean(result)}`,
        "Logic.Equal": (inputs, result) => `(${formatValueForDisplay(inputs[0])} == ${formatValueForDisplay(inputs[1])}) = ${formatBoolean(result)}`,
        ...
        "Vector.Length": (inputs, result) => {
            ...
            // Modified to use consistent formatting
            return `|${formatValueForDisplay(vector)}| = ${formatValueForDisplay(result)}`;
        },
    ...
    "Flow.ForLoop": (firstIndex, lastIndex) => {
        return { firstIndex, lastIndex };
    },
    "Flow.WhileLoop": (condition) => {
        return !!condition;
    },
    "Flow.DoOnce": () => true,
    ...
    "Logic.StrictEqual": (leftOperand, rightOperand) => leftOperand === rightOperand,
    "Logic.StrictNotEqual": (leftOperand, rightOperand) => leftOperand !== rightOperand,
    "Logic.Greater": (leftOperand, rightOperand) => {
        validateNumericComparison(leftOperand, rightOperand);
        const a = coerceNumber(leftOperand);
        const b = coerceNumber(rightOperand);
        return a > b;
    },
    "Logic.GreaterEqual": (leftOperand, rightOperand) => {
        validateNumericComparison(leftOperand, rightOperand);
        const a = coerceNumber(leftOperand);
        const b = coerceNumber(rightOperand);
        return a >= b;
    },
    "Logic.Less": (leftOperand, rightOperand) => {
        validateNumericComparison(leftOperand, rightOperand);
        const a = coerceNumber(leftOperand);
        const b = coerceNumber(rightOperand);
        return a < b;
    },
    "Logic.LessEqual": (leftOperand, rightOperand) => {
        validateNumericComparison(leftOperand, rightOperand);
        const a = coerceNumber(leftOperand);
        const b = coerceNumber(rightOperand);
        return a <= b;
    ...
    "Math.RandomFloatInRange": (minValue, maxValue) => {
        const min = coerceNumber(minValue);
        const max = coerceNumber(maxValue);
        const lo = Math.min(min, max);
        const hi = Math.max(min, max);
        return Math.random() * (hi - lo) + lo;
    },

    "Math.RandomIntInRange": (minValue, maxValue) => {
        const min = Math.ceil(coerceNumber(minValue));
        const max = Math.floor(coerceNumber(maxValue));
        const lo = Math.min(min, max);
        const hi = Math.max(min, max);
...
function formatValueForDisplay(value) {
    ...
    if (typeof value === 'object' && value !== null && 'x' in value) {
        ...
        const z = parseFloat(value.z.toFixed(2));
        return `(${x}, ${y}, ${z})`;
    }
    
    // Format Numbers to 2 decimal places max (stripping trailing zeros)
